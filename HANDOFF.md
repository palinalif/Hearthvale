...
Terrain retains A sculpt and X to edit a hovered cottage; otherwise X opens terrain settings. D-pad Up remains the top-level context switch. In ordinary cottage editing, A on a bare selected wall/roof opens Move house / Resize house; a detail keeps direct A-move and X-options. D-pad chooses, A confirms, B closes the chooser. Empty ground does not trigger house actions or terrain prompts while editing a cottage.
...
3. Review and iterate the higher-detail cottage candidate, then continue with tree/river refinement. The current branch implements the player-approved cottage-only `0.0625` decorative grid for window joinery, shutters, trims, roof edges, flower boxes and flowers, with deterministic craft variations and new headless/actual-Mobile gates. Cottage structure, resize increments and attachment anchors remain at `0.125`; do not apply the finer tier to terrain or other asset families. See `reports/M1-cottage-half-cell-detail.md`. Desktop validation passed; player visual approval and Thor performance remain open. No M2, villagers or expanded building catalogue in this pass.
4. Stable securely retained Android signing and deliberate save migration before routine in-place updates.

Pinned Godot 4.7.2.stable.official.ed1daf0bf, locked templates/native Voxel Tools, `0.125` structural visible grid plus the approved cottage-only `0.0625` decorative tier, miniature scale, world dimensions and schemas remain unchanged. Preserve native caves/overhangs, authoritative IDs/manual/suppressed/unsupported records, transactions/cancellation, player saves and old artifacts. Never weaken validation to produce a green result. Desktop headless/software-render checks are not handheld performance measurements. No local Godot execution or independent reviewer is claimed for this integration.
