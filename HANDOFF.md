# 2026-09-26 — approved 64 m sun shadows, v85 candidate

User preferred the 64 m desktop Mobile capture over 150 m and approved making it the default. Set `WorldSun.directional_shadow_max_distance` to 64 m in `scripts/m1_scene.gd`, retaining sun shadows; updated the comparison tool to restore the scene's actual production setting. The starter-scene test checks the live light and passed 28/28 via `tools/run-test tests/m2_starter_scene_test.gd`. Exported `builds/hearthvale-m2night-v85-debug.apk` from this source: SHA-256 `b356b2e1b3bc1966c6998f1a18d800fb5b94426e30e80920d88731a494d9776a`, package `org.hearthvale.game.test.m2night`, versionCode 85, exactly one ARM64 `libvoxel.android.editor.arm64.so` matching the installed extension (`641f87ad…`), verified by `apksigner` with the existing in-place-compatible debug certificate (`3d88162b…`). `aapt dump badging` printed an Android SDK attribute warning but did report the expected package/version; export and signature checks succeeded. No Thor install, new device FPS measurement, controller test or on-device visual acceptance: Thor was unavailable. Previous live temporary A/B at 64 m was ~23 FPS against ~18 FPS at 150 m, not evidence of this APK's performance. Keep the installed v84 app and saves untouched; when Thor is available, install v85 **in place**, verify package/version and settled FPS and visuals, then close the game for cooldown. `local.properties` is host-only and remains untracked.

# 2026-09-26 — reversible 64 m shadow comparison (desktop only)

Extended `tools/waterfall_shot.gd` with optional `--shadow-pair <prefix>`: one temporary fresh-valley boot, same Mobile-renderer camera, captures at the unchanged 150 m setting then 64 m, and restores 150 m before exit. Ran `--cam 95,40,35 --look 48,8,54 --shadow-pair .tools/shadow-cottage` under Xvfb with failfast: backend, full valley mesh and target-water readiness succeeded; both PNGs saved at `.tools/shadow-cottage-{150m,64m}.png` (ignored). Startup logged the normal ALSA audio-device warning/fallback, not a capture failure. At 64 m the distant right hillside's broad dark shade disappears and the plateau/right valley looks noticeably brighter; near foreground shadows remain. These are desktop Mobile captures of a **fresh** valley, not equivalent to Thor rendering or the live saved world; visual acceptance is the user's. The existing Thor temporary A/B measured ~18 FPS at 150 m versus ~23 FPS at 64 m, but these new captures do not measure FPS. Production sun stays at 150 m, device/saves untouched, no APK built. If the user accepts the visual trade-off, next run an in-place Thor 64 m candidate against the exact source and compare settled FPS and visual presentation; otherwise explore another approach instead of shipping a washed-out valley.

# 2026-09-26 — offline terrain underside/shadow feasibility (diagnostic only)

Extended the ignored fresh-generator probe `.tools/probe_valley_blocks.gd` to classify the triangles in the same 144 sampled 32-cell blocks: **98,304 bottom, 98,304 top, 131,384 side** (327,992 total). All bottom triangles belonged to 48 blocks at the world's y=0 floor. Supplying a *solid one-cell padding below* those blocks **in the probe buffer only** removed precisely those 98,304 bottom triangles (327,992 → 229,688) and reduced mesh surfaces 324 → 282, without changing sampled top/side triangle counts. Logs: `.tools/probe-valley-directions.log`, `.tools/probe-valley-sealed-floor.log` (ignored). This measures generated **mesh geometry**, not submitted Thor draws, shadow primitives or frame time. Backface/shadow culling may already avoid drawing the bottom; extrapolating this sample to the full valley would be misleading. No gameplay voxels or meshes were changed.

Read-only local-Qwen API investigation found no documented `VoxelMesherBlocky` per-direction face suppression or per-face shadow control in the pinned extension (which bundles no API documentation). `occlusion_enabled` only concerns neighbour-facing occlusion and did not remove exterior faces in the earlier local probe. Extending native terrain bounds with filled voxels below y=0 would alter the data/streaming/save boundary and is **not approved or proven safe**. Prior Thor A/B already establishes that *whole-scene* sun shadow distance affects draws/FPS substantially (150 m → 24 m: ~18 → 24 FPS) but changes the valley's lighting visibly; no shadow setting was promoted. Next useful step, when Thor is available, is an **in-place, temporary comparison that isolates actual rendered underside contribution** (or shadow submissions) before implementing a terrain floor/mesher intervention. Do not infer a performance gain from this CPU geometry census. Device untouched; no APK built.

# 2026-09-25 — native block mesh survey (diagnostic only)

Read-only Godot probe `.tools/probe_valley_blocks.gd` reconstructed the **fresh generator's** padded 32-cell native blocks around cottage/meadow/mountain/tunnel plus a deliberately watercourse-heavy 6×7 grid at x=72–92 m, z=120–152 m. It used the pinned `VoxelMesherBlocky` and compared stock library versus the same shared-opaque-material variant tried in v84; the 144 nonempty rendered blocks measured **324 → 218 mesh surfaces** (69 blocks improved), with **327,992 triangles unchanged**. Of those, the watercourse sample contributed 306 → 203 surfaces; the cottage, meadow and mountain samples stayed at one surface each for their visible blocks. This is a biased local sample of generated terrain, **not** an exhaustive count of live device blocks or a saved/edited world. The probe log is `.tools/probe-valley-blocks.log` (ignored). v84's settled Thor rendering still showed exactly **6,464 draws / 17,237,918 primitives and 17–18 FPS**, so a real CPU mesher surface reduction in selected generated blocks does **not** establish fewer runtime draws or better frame time. Do not re-ship material batching from this probe: actual live meshed block materials/residency and draw submission need measurement before another APK trial. No gameplay source, saves or device state changed in this survey.

# 2026-09-25 — v84 opaque-model color batching rejected

A trial moved six opaque terrain colors to `VoxelBlockyModelCube.set_color` and assigned one shared white material, with a passing focused ground-material test and CPU mesher probe. Exported v84 and installed it **in place** on Thor `192.168.1.15:33041` (same signing identity; ARM64 voxel library verified). At the settled starter-valley view v84 measured 17–18 FPS, **6,464 draws and 17,237,918 primitives**, indistinguishable from v83's 17–18 FPS / 6,464 draws / 17,237,918 primitives. Screenshot `.tools/thor-v84-batching.png` versus `.tools/thor-v83-shadow-150m.png` also shows lighting/color differences on the hillside; there is no basis to ship this as either a visual or performance improvement. Reverted the source, test, CI registration and export preset to v83; v84 remains installed on device as a negative experiment only. Do not downgrade by uninstalling or discard saves. Game force-stopped and Chrome resumed after measurement. Next terrain optimization must first explain why changing library materials did not reduce runtime surfaces (e.g. actual mesh material indices or shadow pass cost) and be measured on device before promotion.

# 2026-09-25 — Thor v83 settled terrain rendering diagnostics (no runtime change)

In-place v83 install was verified on Thor `192.168.1.15:33041`; settled native meshing queue was empty. The starter valley ran at ~18 FPS / 17.24M rendered primitives / 6,464 draws; hiding only `VoxelTerrain` raised it to ~60 FPS / 1.70M primitives / 552 draws, and restoring visibility returned to 18 FPS. The scenery MultiMeshes were not the cause. A temporary sun shadow-distance change from 150 m to 24 m yielded 24 FPS / 6.11M primitives / 2,528 draws; disabling the sun's shadows yielded 26–28 FPS / 5.17M primitives / 2,493 draws. Settings were restored, and the game was force-stopped with Chrome resumed for cooldown. The large shadow gain is real, but 24 FPS still misses 30 and changing shadows alters presentation; no runtime setting was promoted.

A further temporary shadow-distance A/B gave 96 m ~20–21 FPS, 64 m ~23 FPS and 48 m ~23–24 FPS; restoring 150 m returned ~18 FPS. These distances were not promoted. The game was stopped and Chrome resumed after the run.

The 150 m and 24 m Thor captures (`.tools/thor-v83-shadow-150m.png`, `.tools/thor-v83-shadow-24m.png`) show a conspicuous lighting change: the long, soft hillside shadows and shade over the near cottage disappear, washing out the valley at 24 m. Do not ship the 24 m setting without the player's visual review. Resolution A/B (physical 1080×1920 versus temporary 720×1280) gave only ~18 versus ~19–20 FPS; the resolution override was reset. The pinned extension does expose `VoxelMesherCubes.greedy_meshing_enabled`, but it reads `VoxelBuffer.CHANNEL_COLOR`, whereas this world's authoritative and saved data are `CHANNEL_TYPE` and use `VoxelMesherBlocky`. It is **not** a drop-in mesher switch; duplicating the large world in a second channel or changing save representation needs memory/edit/save/visual profiling first. Pinned terrain mesh blocks support 16/32 cells, not 64. Do not implement either speculative switch based on a subagent's unverified suggestion. Continue with quantitatively measured terrain geometry/shadow alternatives while preserving the full native valley and caves. Desktop checks cannot establish Thor visual acceptance.

# 2026-09-25 — restored terrain/water edit responsiveness (v83)

Compared `07716bc4`, `ebe597c0`, `4e564115` and `4bbdd2db` against this branch. The startup parent callback from `4bbdd2db` survived; missing terrain/water responsiveness paths and the APK-repair package guard were restored in `3a445b0` without replacing the newer waterfall visuals. Sparse terrain edit tracking/native column sampling, local meadow updates, landing reuse, bounded water surface updates, cheaper previews and excavation sampling, and water-history terrain restoration have focused coverage; new water tests are registered in CI. The missing APK-repair package guard is restored. This addresses *edit stalls*, not the separately measured ~8 fps settled terrain rendering cost.

Godot 4.7.2 focused suites passed: terrain edit cells 14, meadow tuft 18, house edge foliage 33, water history 13, incremental budget 10, water tool responsiveness 9, waterfall visual 19, waterfall cliff shape 1819, premade river 73, starter scene 27; adjacent sculpt 188, sculpt smoothing 23, scaled backend 44, water region 55, carve plan 12, lake outline 6. A separate terrain-resolution suite has 11 pre-existing failures and remains follow-up. `git diff --check` passed. Exported and verified ARM64 `builds/hearthvale-m2night-v83-debug.apk`, SHA-256 `95233f2a79301de73c657073b461775259e8e6e200bd54b98e109264505fb451`, package `org.hearthvale.game.test.m2night`, versionCode 83; exactly one ARM64 voxel library, hash `641f87ad…` matching the pinned installed binary. APK verifies with the existing v81/v82 debug signing certificate `3d88162b…` (the local debug keystore; note the parent workspace instructions list a different `d44a…` fingerprint). Thor install, edits, FPS, controller and visual acceptance **not run** for v83; retain the existing app and saves until an in-place install can be verified.

# 2026-09-24 — terrain rendering A/B, rejected v82 occlusion candidate

Thor v82 visual-viewer diagnostic (2026-09-24): with a settled meshing queue, temporary live radii 285 m (original), 96 m, 64 m and 48 m yielded respectively ~17.6M / 10.2M / 5.97M / 4.19M rendered primitives and 6,473 / 3,475 / 1,778 / 1,206 draws. FPS stayed ~6–8 / 7 / 8 / 8 and process time ~145–171 / 131–134 / 126–128 / 124–127 ms. Restoring the original 285 m radius restored ~17.6M / 6,473 and ~7 fps; the script's restoration succeeded before the shell wrapper returned exit 252 (the cleanup's incorrect Chrome component failed; a follow-up `dumpsys` confirmed the game was closed and `com.android.chrome/.Main` was resumed; battery 30 °C). This **rejects radius clipping as a useful fix**: it removes distant terrain without meaningfully improving FPS. No source gameplay setting was changed. A previous subagent report incorrectly claimed the occlusion flag remains on and the three grass variants share one visually identical material; current `terrain_backend.gd` has `occlusion_enabled = false` after the rejected trial, and `terrain_ground_materials.gd` gives grass variants distinct shader tints. Verify any mesher/model API and quantify boundary-face claims before touching terrain data or save compatibility.\n\nThor v81 settled at 8 fps / 17,595,594 rendered primitives / 6,473 draws with native terrain shown; hiding only `VoxelTerrain` raised it to 14 fps / 2,056,714 primitives / 561 draws, restoring returned to 8 fps. Voxel meshing task queue was empty: this is steady rendering cost, not a pending generation stall. An in-place v82 APK trial changed `VoxelMesherBlocky.occlusion_enabled` from false to true. After full load it gave **the same 8 fps / 17,595,576 primitives / 6,473 draws**; hiding terrain gave 16 fps / 2,056,696 primitives / 561 draws. A padded CPU cube mesher probe in `.tools/probe_cube_occlusion.gd` produced exactly 128 triangles with both settings for a solid-layer sample, confirming that covered neighbor faces were already culled (the default value of this setting is `true`; it is not a geometry-culling toggle for these cubes). The ineffective flag change was reverted in source; v82 remains installed on Thor only as a tested negative experiment. Source preset returned to v81; **do not treat the v82 APK as a performance fix or deliver it**. `tests/m1_scaled_backend_test.gd` updates stale 80 m fixtures for the existing 160 m valley. The updated scaled-backend test passed 44/44 after the revert; current ground-material (87), waterfall-visual (19), valley-ring (735,842) and starter-scene (27) checks passed with the trial enabled. Focus next on measured visible terrain geometry/draw batching while keeping native volumetric terrain and distant scenery. Screenshot `.tools/thor-v82-occlusion.png` shows waterfall and valley still present; no user visual approval. APK v82 had exactly one ARM64 `libvoxel` and was signed with the same `3d88162b…` key as installed v81; SHA-256 `98483bc400a60a2b6178ebc045f359bba3ccdbb0a80b84f2067b4df4a0903d6e`. Thor pairing `192.168.1.15:33041`, battery 31 °C before trial; app force-stopped and Chrome screensaver resumed after it. Saves retained. No final on-device sculpt/boundary acceptance yet.

# 2026-09-24 — Thor pairing and v81 signing blocker

Wireless ADB paired at `192.168.1.15:33041` (port may change). Installed `org.hearthvale.game.test.m2night` is v76. In-place v81 install initially failed with `INSTALL_FAILED_UPDATE_INCOMPATIBLE`: installed v76 used the lost old signing key, while v81 uses the new key. User explicitly approved removal of v76 and its unneeded saves. Uninstalled **only** `org.hearthvale.game.test.m2night` v76, then installed verified v81 APK (`4e482392…`) successfully; `dumpsys package` reports versionCode 81. Thor smoke check: fresh starter valley rendered with native voxel terrain, river, houses and distant waterfall (`.tools/thor-v81-loaded.png`; earlier partial first-boot frame was captured during loading). Debug bridge reported 53 scene meshes / 336,540 triangles; controller left-stick input moved cursor `[48,8,54]` → `[42.89,8,48.61]` with no stroke, and bridge state showed menus closed. Telemetry reported **7 fps** both after loading and after cursor input; 32 °C battery, ~3.0 GB PSS. This is a serious performance concern, not a successful FPS acceptance test; full meshing/performance diagnosis and actual sculpt/boundary interaction were not run. No fatal/script errors found in sampled logcat. Game force-stopped and Chrome launched for cooldown/burn-in protection. User visual acceptance remains outstanding.

# 2026-09-23 — v81 scenery editing footprints

The starter-valley rim now checks whole edit footprints: sculpt/plant brush disks, painted path disks and continuous strokes, and rotated home footprints (including multi-section bounds). A held sculpt brush slides at the inner rim. Existing home records and saves are not relocated; only new edits are gated. Legacy placement/path test fixtures were moved into the playable valley so their original behavior is still exercised. New footprint geometry tests are registered in water CI.

Godot 4.7.2 focused checks passed: footprint geometry 37, scene integration 17, placement rotation 27, path placement 57, path history 13, path edit reach 14, legacy building placement 17, path cancel feedback 16. The earlier regression batch failed only on old fixtures outside the boundary; each affected test subsequently passed after fixture changes. `git diff --check` passed. Exported `builds/hearthvale-m2night-v81-debug.apk` (SHA-256 `4e4823920e3451a6032941f48fd861e7b553644cad58a565d56602a363b4110c`), package `org.hearthvale.game.test.m2night`, versionCode 81, signed with the v80 debug certificate (`3d88162b…`); exactly one ARM64 libvoxel, byte-for-byte matching the installed extension (`641f87ad…`). Thor installation, controller/edit behavior, on-device rendering and visual acceptance **not tested**; ADB daemon was unavailable at export. Do not uninstall the app or discard saves. `local.properties` is untracked host configuration.
