...
## Bob — step 4 unit 5 (2026-09-15): gates, draw calls, captures, push

**Branch `task/visual-overhaul-1`. This entry's commit is the pushed tip (a commit
cannot cite its own sha); `git log --oneline -6` + `git ls-remote` confirm.**

**QUICK GATE (the brief's non-negotiable, `bash tools/run_render_gate.sh --quick`):**
```
...
m2_hamlet_composition_render_test exit=0 checks=12 failures=0
```
= exactly the required 21/0, 4/0, 33/0, 12/0. **FULL GATE (default mode, mobile
renderer, all 5 suites) also run:** nav 21/0, boot 4/0, cottage 33/0, facade 54/0,
hamlet 12/0, `joined_roof_course_render_test` **80/4** — the four FAILs are all
`"unchanged roof remains pixel-stable"`, the known pre-existing sub-quantisation float
drift. **Re-measured at the base commit 8991a4c in this run: also 80 checks / 4
failures, same labels** — i.e. not caused by anything here.
Terrain/contract tests: `m1_riverbank_visual_test` 204/0, `terrain_resolution_test`
46/0, `m1_scaled_backend_test` 29/0, `m1_landscape_test` 128/0, `foliage_asset_test`
57/0.

**DRAW CALLS (measured with `tools/bob_drawcall_probe.gd`, same scene + framing,
base 8991a4c vs this tip):**
```
                      base 8991a4c      tip
draw_calls            1155              1127     (no increase; -28, culling noise)
primitives            3,047,926         3,142,238  (+3.1%)
objects_in_frame      1238              1210
plant_instances       1031              1189     (+158, same 67 MultiMesh batches)
records               262               301
```
The backdrop adds exactly ONE merged surface (the outer valley floor, colour index 4),
so by construction it is +1 draw call; the frame total measured *lower*, i.e. there is
no draw-call regression from this work. Mobile renderer, no volumetrics/SDFGI/SSR;
vegetation still instanced only; the 30 FPS budget is not measurably affected by a
+3% primitive static mesh with shadow casting off.

**CAPTURES (regenerated by the gate / the rim probe, diffable):**
```
reports/screenshots/step4-before/full-hamlet-before.png       pre-fix wide shot
reports/screenshots/step4-before/01-normal-clean-before.png   pre-fix cottage detail
reports/screenshots/step4-after/full-hamlet-after.png         post-fix wide shot
reports/screenshots/step4-after/01-normal-clean-after.png     post-fix cottage detail
reports/screenshots/m2-hamlet/full-hamlet.png                 (gate capture, = after)
reports/screenshots/m1-cottage-detail/*.png                   (gate captures, = after)
reports/screenshots/step4-rim/<dir>-before.png  + -before-void.png
reports/screenshots/step4-rim/<dir>-after.png   + -after-void.png   8 directions each
```

**For the next run / for Pali:** (a) the only known-open gate failure is the
pre-existing `joined_roof_course_render_test` 80/4 float drift (unchanged, not chased);
