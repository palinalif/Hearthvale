	...
	if node is MultiMeshInstance3D:
		...
		if not rendered:
			...
			for i in multi.instance_count:
				...
				_inspect_mesh(multi.mesh, instance, origin, label + "/" + str(node.name))
				instances_checked += 1
	elif node is MeshInstance3D and node.mesh:
		_inspect_mesh(node.mesh, node.global_transform, origin, label + "/" + str(node.name))
	for child in node.get_children(): _inspect_node(child, origin, label, rendered)

func _inspect_mesh(mesh: Mesh, transform_value: Transform3D, origin: Vector3, label: String) -> void:
	for surface in mesh.get_surface_count():
		...
		for vertex in vertices:
			...
			for axis in 3:
				on_grid = on_grid and absf(world_point[axis] / Grid.UNIT - roundf(world_point[axis] / Grid.UNIT)) < 0.0002
		vertices_checked += vertices.size()
		_check(on_grid, label + " world vertices share %.3f grid" % Grid.UNIT)
