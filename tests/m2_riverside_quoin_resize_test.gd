...
func _run() -> void:
	...
	var building_id := scene.selected_building_id
	...
	var taller := original_dimensions
	taller.y = 8.0
	...
	var one_level_count := _extra_quoin_instance_count(building_id)
	...
	scene._refresh_raised_foundation_masonry(true)
	var two_level_count := _extra_quoin_instance_count(building_id)
...
func _extra_quoin_instance_count(building_id: String) -> int:
	...
	var courses := root_node.get_node_or_null("ExtraQuoinCourses") as MultiMeshInstance3D
