...
func _run() -> void:
	...
	check(scene._building_panel.visible and _focus_text().begins_with("Duplicate"), "physical X opens cottage options")
	await _press(JOY_BUTTON_DPAD_DOWN)
	check(_focus_text() == "Add flower box", "first Down follows displayed flower-box row")
	await _press(JOY_BUTTON_DPAD_DOWN)
	check(_focus_text() == "Add shutter", "second Down follows shutter row")
	await _press(JOY_BUTTON_DPAD_DOWN)
	check(_focus_text().begins_with("Material:"), "third Down follows material row")
	await _press(JOY_BUTTON_DPAD_DOWN)
	check(root.gui_get_focus_owner() == scene._needs_action, "fourth Down reaches Needs placement BEFORE Close")
	await _press(JOY_BUTTON_DPAD_DOWN)
