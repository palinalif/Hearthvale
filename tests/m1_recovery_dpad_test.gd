...
func _run() -> void:
	...
	check(actual_rows.size() == expected_rows.size(), "compact house menu exposes only current management rows")
	for index in mini(actual_rows.size(), expected_rows.size()):
		check(actual_rows[index].begins_with(expected_rows[index]), "displayed house row %d is %s" % [index, expected_rows[index]])
	check(root.gui_get_focus_owner() == scene._building_buttons[0], "house management opens focused on first displayed row")
	for index in range(1, scene._building_buttons.size()):
		await _press(JOY_BUTTON_DPAD_DOWN)
		check(root.gui_get_focus_owner() == scene._building_buttons[index], "Down follows displayed house row %d" % index)
	check(_focus_text() == "Close", "Close is the final displayed house row")
