...
func _run() -> void:
	...
	await _press(JOY_BUTTON_DPAD_DOWN)
	check(_focus_text() == "Add flower box", "second Down follows displayed flower-box row")
	await _press(JOY_BUTTON_DPAD_DOWN)
	check(_focus_text() == "Add shutter", "third Down follows shutter row")
	await _press(JOY_BUTTON_DPAD_DOWN)
	check(_focus_text().begins_with("Wall material"), "fourth Down follows wall material row")
	await _press(JOY_BUTTON_DPAD_DOWN)
	check(_focus_text().begins_with("Roof material"), "fifth Down follows roof material row")
	await _press(JOY_BUTTON_DPAD_DOWN)
	check(root.gui_get_focus_owner() == scene._needs_action, "sixth Down reaches Needs placement before Close")
	await _press(JOY_BUTTON_DPAD_DOWN)
