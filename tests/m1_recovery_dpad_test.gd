...
func _run() -> void:
	...
	await _press(JOY_BUTTON_DPAD_DOWN)
	check(_focus_text().begins_with("Wall material"), "sixth Down follows wall material row")
	await _press(JOY_BUTTON_DPAD_DOWN)
	check(_focus_text().begins_with("Roof material"), "seventh Down follows roof material row")
	await _press(JOY_BUTTON_DPAD_DOWN)
	check(root.gui_get_focus_owner() == scene._needs_action, "eighth Down reaches Needs placement before Close")
	await _press(JOY_BUTTON_DPAD_DOWN)
