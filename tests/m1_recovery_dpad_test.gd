...
func _run() -> void:
	...
	await _press(JOY_BUTTON_DPAD_DOWN)
	check(_focus_text() == "Add window", "second Down follows displayed window row")
	await _press(JOY_BUTTON_DPAD_DOWN)
	check(_focus_text() == "Add door", "third Down follows displayed door row")
	await _press(JOY_BUTTON_DPAD_DOWN)
	check(_focus_text() == "Add flower box", "fourth Down follows displayed flower-box row")
	await _press(JOY_BUTTON_DPAD_DOWN)
	check(_focus_text() == "Add shutter", "fifth Down follows shutter row")
	await _press(JOY_BUTTON_DPAD_DOWN)
	check(_focus_text() == "Wall colour", "sixth Down follows wall colour row")
	await _press(JOY_BUTTON_DPAD_DOWN)
	check(_focus_text() == "Roof colour", "seventh Down follows roof colour row")
	await _press(JOY_BUTTON_DPAD_DOWN)
	check(_focus_text() == "Roof design", "eighth Down follows roof design row")
	await _press(JOY_BUTTON_DPAD_DOWN)
	check(_focus_text() == "Roof decor", "ninth Down follows roof decor row")
	await _press(JOY_BUTTON_DPAD_DOWN)
	check(_focus_text() == "Accent colour", "tenth Down follows shared accent colour row")
	await _press(JOY_BUTTON_DPAD_DOWN)
	check(root.gui_get_focus_owner() == scene._needs_action, "eleventh Down reaches Needs placement before Close")
	await _press(JOY_BUTTON_DPAD_DOWN)
