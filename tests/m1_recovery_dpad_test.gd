...
func _run() -> void:
	...
	await _press(JOY_BUTTON_DPAD_DOWN)
	check(_focus_text() == "Accent colour", "eighth Down follows shared accent colour row")
	await _press(JOY_BUTTON_DPAD_DOWN)
	check(root.gui_get_focus_owner() == scene._needs_action, "ninth Down reaches Needs placement before Close")
	await _press(JOY_BUTTON_DPAD_DOWN)
