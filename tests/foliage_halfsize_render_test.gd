...
func _run() -> void:
	...
	for variant in Flora.variant_count("foliage"):
		...
		for mesh: Mesh in Flora.meshes("foliage", variant):
			var part := MeshInstance3D.new(); part.mesh = mesh; part.position = position; scene.add_child(part)
	...
	var title := Label.new(); title.text = "HEARTHVALE / HALF-SIZE FOLIAGE / MOBILE REVIEW"; title.position = Vector2(28, 20); title.add_theme_font_size_override("font_size", 25); title.modulate = Color("#26392f"); overlay.add_child(title)
	var note := Label.new(); note.text = "0.0625 cubic cells  •  cream mushroom caps above brown stems  •  0.125 terrain cube at right"; note.position = Vector2(28, 54); note.add_theme_font_size_override("font_size", 17); note.modulate = Color("#3f594a"); overlay.add_child(note)
