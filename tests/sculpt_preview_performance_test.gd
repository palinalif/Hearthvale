	...
	for radius in [2.0, 8.0]:
		for phase in ["cold", "moving", "held"]:
			...
			for sample in 5:
				...
				check(not plan.is_empty(), "worker finishes bounded request")
			...
			check(_percentile(capture_times, 0.8) < 12.0, "snapshot service stays below 12ms p80")
			check(_percentile(service_times, 0.95) < 2.0, "nonblocking poll p95 below 2ms")
			check(_percentile(publish_times, 0.8) < 4.0, "bulk publication below 4ms p80")
			print("PREVIEW_PERF " + JSON.stringify({"radius": radius, "phase": phase, "samples": latencies.size(), "cells": cell_count, "snapshot_bytes": job.last_snapshot_bytes, "capture_ms": capture_times, "poll_p95_ms": _percentile(service_times, 0.95), "upload_ms": publish_times, "worker_query_ms": worker_times, "worker_pack_ms": pack_times, "response_ms": latencies}))
			job.close()
