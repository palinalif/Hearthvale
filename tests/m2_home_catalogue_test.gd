...
func _initialize() -> void:
	...
	check(scene._build_catalogue_buttons.size() == 4 and scene._build_catalogue_buttons[0].text.begins_with("Buildings") and scene._build_catalogue_buttons[1].text.begins_with("Roads & paths") and scene._build_catalogue_buttons[2].text.begins_with("Outdoor decorations") and scene._build_catalogue_buttons[3].text.begins_with("Hamlet details"), "build catalogue exposes clear top-level categories")
	...
	scene._open_hamlet_catalogue()
	check(scene._hamlet_catalogue_open and scene._hamlet_catalogue_panel.visible and scene._hamlet_catalogue_buttons.size() == 9, "hamlet details category exposes gardens fences and street furniture")
	check(scene._hamlet_catalogue_buttons[0].text.begins_with("Cottage flower garden") and scene._hamlet_catalogue_buttons[3].text.begins_with("Rustic timber fence") and scene._hamlet_catalogue_buttons[4].text.begins_with("Rustic garden gate") and scene._hamlet_catalogue_buttons[5].text.begins_with("Village bench") and scene._hamlet_catalogue_buttons[8].text.begins_with("Barrel planter"), "hamlet composition choices are visually named and ordered")
	check(scene._hamlet_catalogue_panel.get_global_rect().end.y <= scene._prompt_bar.get_global_rect().position.y + 1.0, "complete hamlet catalogue stays above the controller prompt bar")
	scene._return_to_build_catalogue()
	...
	var initial_wall: String = str(scene._catalogue_wall_choices["woodland_lodge"])
	var initial_roof: String = str(scene._catalogue_roof_choices["woodland_lodge"])
	...
	var expected_wall: String = str(scene.WALL_MATERIALS[posmod(scene.WALL_MATERIALS.find(initial_wall) + 1, scene.WALL_MATERIALS.size())])
	var expected_roof: String = str(roof_choices[posmod(roof_choices.find(initial_roof) + 1, roof_choices.size())])
	...
	var alternate_roof: String = _different_choice(roof_choices, chosen_roof)
