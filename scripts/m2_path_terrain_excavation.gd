...
## Plan only the additional excavation needed when an already-cut packed-earth
## mask grows. Existing cells use their previous profile depth as owned terrain,
## so repainting/expanding does not repeatedly dig the current surface.
static func plan_packed_earth_transition(backend: Object, before_values: Array, after_values: Array) -> Dictionary:
	if backend == null or not backend.has_method("voxel_at"):
		return _empty("terrain backend unavailable")
	var scale := float(backend.get("voxel_scale"))
	var patch: Vector3i = backend.get("patch_size")
	...
	for cell: Vector2i in after_cells:
		...
		for offset in delta:
			...
			var material := int(backend.voxel_at(position))
			if material == 0: break
			removals.append({"position": position, "before": material, "after": 0, "depth": after_depth, "delta": delta})
			min_pos = min_pos.min(position)
			max_pos = max_pos.max(position + Vector3i.ONE)
	return {
		"ok": true,
