...
static func paint(path_values: Array, next_id: int, style_id: String, cell_values: Array) -> Dictionary:
	var original_next_id := next_id
	if not STYLE_IDS.has(style_id): return _result(path_values, original_next_id, false, -1)
	var painted := Region.normalize_cells(cell_values)
	if painted.is_empty(): return _result(path_values, original_next_id, false, -1)
	...
	for path_value in path_values:
		...
		for cell: Vector2i in cells:
			if paint_lookup.has(cell): changed = true
		...
		elif not cells.is_empty(): changed = true
	...
	if total_cells(result_paths) > MAX_CELLS: return _result(path_values, original_next_id, false, -1)
...
static func validate(path_values: Array) -> bool:
	...
	for path_value in path_values:
		...
		if not _is_integer_number(id_value): return false
		var id := int(id_value)
		...
		for index in cells.size():
			...
			if not _is_integer_number(raw[0]) or not _is_integer_number(raw[1]): return false
