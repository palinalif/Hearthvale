	...
	for index in samples.size():
		...
		for across: float in ACROSS:
			...
			if index == 0 or index == samples.size() - 1:
				...
				end_shift = minf(end_length * 0.20, minf(0.12, safe_width * 0.16)) * absf(across) * (0.85 + 0.15 * across)
...
static func extent(width: float, distance: float, path_id: int, side: int) -> float:
	# Continuous distance, not unrelated segment random numbers. Never exceeds
	# nominal half-width; less than 6% total width loss even at narrow settings.
	var phase := float(Contact._seed(path_id, 0, 11 + side) % 6283) / 1000.0
...
static func _emit_soil(builder: Dictionary, polygon: PackedVector2Array, height: float, path_id: int, station_a: Dictionary, station_b: Dictionary) -> void:
	...
	for point: Vector2 in polygon:
		...
		var edge := clampf(point.distance_to(closest) / (float(station_a["width"]) * 0.5), 0.0, 1.0)
