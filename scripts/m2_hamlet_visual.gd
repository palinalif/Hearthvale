...
const FURNITURE_COLOURS := {
	...
	"village_table": TableAssets.SWATCHES,
	"clothesline": TableAssets.SWATCHES,
	"potted_trio": TableAssets.SWATCHES,
	"market_crate": TableAssets.SWATCHES,
	"bird_feeder": TableAssets.SWATCHES,
	"mailbox": TableAssets.SWATCHES,
	"topiary_pair": [Color("#6f8f5f"), Color("#9ab278"), Color("#8c8c84")],
	"beehive": [Color("#c9a878"), Color("#a8815c"), Color("#594337")],
	"wheelbarrow": [Color("#8c6548"), Color("#d89a9a"), Color("#e8dcc0")],
	"flower_arch": [Color("#8c6548"), Color("#d89a9a"), Color("#e8dcc0")],
	"garden_gnome": [Color("#b0604a"), Color("#c9a878"), Color("#e8dcc0")],
	"garden_gnome_small": [Color("#b0604a"), Color("#c9a878"), Color("#e8dcc0")],
	"garden_gnome_tall": [Color("#b0604a"), Color("#c9a878"), Color("#e8dcc0")],
...
func rebuild_furniture(composition_values: Array, terrain_backend: Node = null) -> void:
	...
	for value in composition_values:
		...
		var authored: GDScript = _authored_assets(style_id)
		var mesh: ArrayMesh
		if authored:
			mesh = authored.mesh_for(style_id, str(record.get("colour_id", ""))) as ArrayMesh
			_furniture_geometry_cells += int(authored.VOXEL_COUNTS[style_id])
		else:
		...
		if authored:
			...
			node.set_meta("authored_asset", authored.PATHS[style_id])
		add_child(node)
...
func show_furniture_preview(style_id: String, point: Vector2, size: Vector2, yaw_degrees: float, valid: bool, colour_id: String = "") -> void:
	...
	var mesh: ArrayMesh
	if authored:
		mesh = authored.mesh_for(style_id, colour_id, true, valid) as ArrayMesh
	else:
		var builder := _new_builder(4)
		_append_furniture(builder, record)
	...
	if authored:
		...
		_furniture_preview_node.set_meta("authored_asset", authored.PATHS[style_id])
