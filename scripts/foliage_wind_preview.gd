...
const NAMES := ["grass", "wildflowers", "leafy"]
const STRENGTHS := [0.035, 0.025, 0.03]
...
func _ready() -> void:
	...
	plane.size = Vector2(18, 8) if context else Vector2(6.5, 4.5); ground.mesh = plane
	...
	for i in 3:
		...
		plant.position = Vector3((i - 1) * (5.25 if context else 1.9), 0, 1.4 if context else 1.0)
		add_child(plant); trees.append(plant); winds.append(Wind.new(plant, [0.3, 2.2, 4.4][i]))
		if not context:
			for mesh: Mesh in Flora.meshes("foliage", i):
				var old := MeshInstance3D.new(); old.mesh = mesh
				old.position = Vector3((i - 1) * 1.9, 0, -1.0); add_child(old)
		else:
	...
	if context:
		...
		camera.size = 3.6; camera.look_at_from_position(Vector3(0.8, 4, 6), Vector3(0, 0.2, 0))
	camera.current = true; add_child(camera)
	var overlay := CanvasLayer.new(); add_child(overlay)
	_label(overlay, "HEARTHVALE / " + ("A MEADOW BREEZE" if context else "GROUND FOLIAGE / CURRENT & NEW"), Vector2(30, 24), 26)
	if not context:
		for i in 3:
			var old_point := camera.unproject_position(Vector3((i - 1) * 1.9, 0, -1.0))
			_label(overlay, "CURRENT", old_point + Vector2(-44, 22), 16)
			var point := camera.unproject_position(trees[i].position)
			_label(overlay, ["NEW GRASS", "NEW WILDFLOWERS", "NEW LEAFY PLANT"][i], point + Vector2(-90, 32), 18)
