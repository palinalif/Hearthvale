...
const NAMES := ["reeds", "fern", "mushrooms"]
const STRENGTHS := [0.055, 0.025]
...
func _ready() -> void:
	...
	plane.size = Vector2(18, 8) if context else Vector2(6.5, 3.5); ground.mesh = plane
	...
	for i in 3:
		...
		plant.position = Vector3((i - 1) * (5.25 if context else 1.9), 0, 1.4 if context else 0.0)
		add_child(plant)
		if i < 2:
			trees.append(plant); winds.append(Wind.new(plant, [0.3, 2.2][i]))
		else: rocks.append(plant) # Static mushrooms share the static-asset test contract.
		if context:
			...
			for j in 2:
				...
				add_child(extra)
				if variant < 2:
					context_winds.append(Wind.new(extra, i * 1.8 + j * 0.9))
	...
	if context:
		...
		camera.size = 3.8; camera.look_at_from_position(Vector3(0.8, 4, 6), Vector3(0, 0.65, 0))
	camera.current = true; add_child(camera)
	var overlay := CanvasLayer.new(); add_child(overlay)
	_label(overlay, "HEARTHVALE / " + ("WOODLAND & RIVERSIDE / AT TREE SCALE" if context else "REEDS / FERNS / MUSHROOMS"), Vector2(30, 24), 26)
	if not context:
		for i in 3:
			var point := camera.unproject_position(Vector3((i - 1) * 1.9, 0, 0))
			_label(overlay, ["RIVER REEDS", "WOODLAND FERN", "LITTLE MUSHROOMS"][i], point + Vector2(-80, 38), 18)
