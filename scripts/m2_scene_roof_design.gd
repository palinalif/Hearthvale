...
func _install_roof_design_action() -> void:
	...
	for button in _building_buttons:
		if button.text == "Roof colour":
			roof_colour = button
			break
	if roof_colour:
		box.move_child(_roof_design_button, roof_colour.get_index() + 1)
		var roof_index := _building_buttons.find(roof_colour)
		_building_buttons.insert(roof_index + 1, _roof_design_button)
	else:
		_building_buttons.append(_roof_design_button)
	for button in _building_buttons:
...
func _build_roof_design_picker() -> void:
	...
	for design in ROOF_DESIGNS:
		...
		button.text = "%s  •  %s" % [str(design["label"]), str(design["summary"])]
		button.alignment = HORIZONTAL_ALIGNMENT_LEFT
		button.custom_minimum_size = Vector2(450, 42)
		button.focus_mode = Control.FOCUS_ALL
		button.set_meta("roof_profile", str(design["id"]))
		button.focus_entered.connect(_preview_roof_design.bind(str(design["id"])))
		button.pressed.connect(_commit_roof_design.bind(str(design["id"])))
		box.add_child(button)
		_roof_design_buttons.append(button)
...
func _input(event: InputEvent) -> void:
	if _roof_design_picker_open and not menu_open:
		if event.is_action_pressed("m1_cancel") or event.is_action_pressed("m1_tools"):
			...
			if focus in _roof_design_buttons: (focus as Button).pressed.emit()
		elif event.is_action_pressed("m1_height_up") or event.is_action_pressed("ui_up"):
			_move_focus(_roof_design_buttons, -1)
...
func _open_roof_design_picker() -> void:
	...
	_roof_design_original = str(view.get("roof_profile", "gentle_gable"))
	_roof_design_preview = _roof_design_original
	_roof_design_picker_open = true
	tools_open = true
	...
	for button in _roof_design_buttons:
		if str(button.get_meta("roof_profile", "")) == _roof_design_original:
			focus_button = button
			break
	...
	_set_status("Choose roof design • browse to preview • A apply / B cancel")
	_refresh_controller_hud()

func _preview_roof_design(profile: String) -> void:
	if not _roof_design_picker_open: return
	_roof_design_preview = profile
	_apply_roof_design_preview()
	_set_status("Roof preview: %s • A apply / B cancel" % _roof_label(profile))
...
func _commit_roof_design(profile: String) -> void:
	if not _roof_design_picker_open: return
	var changed := _set_roof_profile(selected_building_id, profile)
	_close_roof_design_picker()
	if changed: _record_history("building")
	_presentation_key = ""
	_update_presentation()
	_set_status("Roof design updated" if changed else "Roof design unchanged")
...
func _cancel_roof_design_picker() -> void:
	...
	_presentation_key = ""
	_update_presentation()
	_set_status("Roof design change cancelled")

func _close_roof_design_picker() -> void:
	_roof_design_picker_open = false
	_roof_design_original = ""
	_roof_design_preview = ""
	tools_open = false
...
func _refresh_roof_overlays() -> void:
	...
	for visual_value in cottage_visuals.values():
		if visual_value is Node3D and is_instance_valid(visual_value):
			...
			if applied is Dictionary: _refresh_roof_overlay_for_visual(visual, applied as Dictionary)
	if cottage_visual is Node3D and is_instance_valid(cottage_visual) and not seen.has((cottage_visual as Node3D).get_instance_id()):
		var applied = (cottage_visual as Node3D).get("_applied_view")
		if applied is Dictionary: _refresh_roof_overlay_for_visual(cottage_visual as Node3D, applied as Dictionary)
...
func _refresh_roof_overlay_for_visual(visual: Node3D, view: Dictionary) -> void:
	...
	var custom := profile in CUSTOM_ROOF_PROFILES
	for child in visual.get_children():
		if _is_base_roof_node(str(child.name)):
			(child as Node).set("visible", not custom)
	if not custom:
		_remove_roof_overlay(visual)
		_roof_overlay_signatures.erase(visual.get_instance_id())
		return
	var signature := "%s|%s|%s|%s" % [profile, str(view.get("dimensions", Vector3.ZERO)), str(view.get("roof_material_id", "terracotta")), str(view.get("wall_material_id", "stone_plaster"))]
	var key := int(visual.get_instance_id())
	var overlay := visual.get_node_or_null("M2RoofDesign") as Node3D
	if str(_roof_overlay_signatures.get(key, "")) == signature and overlay != null: return
	_roof_overlay_signatures[key] = signature
	_remove_roof_overlay(visual)
	_build_custom_roof(visual, view, profile)
...
func _build_custom_roof(visual: Node3D, view: Dictionary, profile: String) -> void:
	...
	var wall_id := str(view.get("wall_material_id", "stone_plaster"))
	var roof_colour: Color = SURFACE_MATERIAL_COLOURS.get(roof_id, Color("#b9654c"))
	var wall_colour: Color = SURFACE_MATERIAL_COLOURS.get(wall_id, Color("#e7cfab"))
	var unit_value = visual.get("_detail_unit")
	var unit: Vector3 = unit_value if unit_value is Vector3 else Vector3.ONE * 0.5
	var root := Node3D.new()
	root.name = "M2RoofDesign"
...
func _build_hip_roof(root: Node3D, dimensions: Vector3, colour: Color, unit: Vector3) -> void:
	var half_x := dimensions.x * 0.5 + 0.5
	var half_z := dimensions.z * 0.5 + 0.5
	var run_step := maxf(unit.z * 2.0, 0.5)
	var height_step := maxf(unit.y, 0.25)
	var rows := maxi(2, ceili(half_z / run_step))
	for row in rows:
		var inset := minf(half_z - unit.z, float(row) * run_step)
		var size_x := maxf(unit.x * 2.0, (half_x - inset) * 2.0)
		var size_z := maxf(unit.z * 2.0, (half_z - inset) * 2.0)
...
func _build_shed_roof(root: Node3D, dimensions: Vector3, colour: Color, wall_colour: Color, unit: Vector3) -> void:
	var half_z := dimensions.z * 0.5 + 0.5
	var run_step := maxf(unit.z * 2.0, 0.5)
	var rows := maxi(2, ceili((half_z * 2.0) / run_step))
	for row in rows:
		var z := -half_z + (row + 0.5) * run_step
		var height := dimensions.y + (row + 0.5) * unit.y * 0.55
		_add_roof_box(root, "Shed_%d" % row, Vector3(0, height, z), Vector3(dimensions.x + 1.0, unit.y, run_step + unit.z * 0.35), colour.lightened(float(row % 2) * 0.03), unit)
		_add_end_infill(root, "ShedFill", dimensions, z, height, run_step, wall_colour, unit)

func _build_saltbox_roof(root: Node3D, dimensions: Vector3, colour: Color, wall_colour: Color, unit: Vector3) -> void:
	var half_z := dimensions.z * 0.5 + 0.5
	var ridge_z := -dimensions.z * 0.16
	var rise := maxf(2.0, dimensions.y * 0.46)
	for side in [-1.0, 1.0]:
		var run := (ridge_z + half_z) if side < 0.0 else (half_z - ridge_z)
		var run_step := maxf(unit.z * 2.0, 0.5)
		var rows := maxi(2, ceili(run / run_step))
		for row in rows:
			var distance := minf(run, (row + 0.5) * run_step)
			var z := ridge_z + side * distance
			var height := dimensions.y + rise * (1.0 - distance / run)
			_add_roof_box(root, "Saltbox_%s_%d" % [side, row], Vector3(0, height, z), Vector3(dimensions.x + 1.0, unit.y, run_step + unit.z * 0.35), colour.lightened(float(row % 2) * 0.03), unit)
			_add_end_infill(root, "SaltboxFill", dimensions, z, height, run_step, wall_colour, unit)
...
func _build_gambrel_roof(root: Node3D, dimensions: Vector3, colour: Color, wall_colour: Color, unit: Vector3) -> void:
	var half_z := dimensions.z * 0.5 + 0.5
	var rise := maxf(2.4, dimensions.y * 0.52)
	var run_step := maxf(unit.z * 2.0, 0.5)
	var rows := maxi(3, ceili(half_z / run_step))
	for side in [-1.0, 1.0]:
		for row in rows:
			var distance := minf(half_z, (row + 0.5) * run_step)
			var t := clampf(distance / half_z, 0.0, 1.0)
			var fraction := lerpf(1.0, 0.68, t / 0.45) if t <= 0.45 else lerpf(0.68, 0.0, (t - 0.45) / 0.55)
			var z := side * distance
			var height := dimensions.y + rise * fraction
			_add_roof_box(root, "Gambrel_%s_%d" % [side, row], Vector3(0, height, z), Vector3(dimensions.x + 1.0, unit.y, run_step + unit.z * 0.35), colour.lightened(float(row % 2) * 0.03), unit)
			_add_end_infill(root, "GambrelFill", dimensions, z, height, run_step, wall_colour, unit)
...
func _add_end_infill(root: Node3D, prefix: String, dimensions: Vector3, z: float, roof_height: float, depth: float, colour: Color, unit: Vector3) -> void:
	var fill_height := roof_height - dimensions.y
	if fill_height <= unit.y * 0.25: return
	for side in [-1.0, 1.0]:
		_add_roof_box(root, "%s_%s_%s" % [prefix, side, str(z)], Vector3(side * dimensions.x * 0.5, dimensions.y + fill_height * 0.5, z), Vector3(unit.x, fill_height, depth), colour, unit)

func _add_roof_box(parent: Node3D, node_name: String, center: Vector3, size: Vector3, colour: Color, unit: Vector3) -> void:
	var quantized: Dictionary = RoofGrid.quantized_box(center, size, unit)
...
func _refresh_controller_hud() -> void:
	...
	if _roof_design_picker_open:
		...
		_tool_name.text = "Roof design"
		_tool_meta.text = _roof_label(_roof_design_preview)
