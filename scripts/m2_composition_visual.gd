...
var _stats := {"bridge_count": 0, "garden_count": 0, "geometry_cells": 0, "bridge_geometry_cells": 0, "garden_geometry_cells": 0, "preview_cells": 0}
...
func show_bridge_preview(style_id: String, width: float, start: Vector2, finish: Vector2, valid: bool) -> void:
	...
	var colours := _preview_colours(BRIDGE_COLOURS[style_id], valid)
...
func _update_total_cells() -> void:
	_stats["geometry_cells"] = int(_stats.get("bridge_geometry_cells", 0)) + int(_stats.get("garden_geometry_cells", 0))

func _preview_colours(base_values: Array, valid: bool) -> Array:
	var tint := Color("#a7e0a0") if valid else Color("#ef8b78")
	var result: Array = []
	for colour_value in base_values:
		result.append((colour_value as Color).lerp(tint, 0.56))
	return result
...
func _append_garden(builder: Dictionary, record: Dictionary) -> void:
	...
	var yaw := posmod(int(record.get("yaw_quarters", 0)), 4)
	var basis := Basis(Vector3.UP, float(yaw) * PI * 0.5)
	var y := _surface_height(point)
...
func _append_cottage_flower_garden(builder: Dictionary, center: Vector3, size: Vector2, basis: Basis) -> void:
	...
	for index in 15:
		...
		var bloom := center + basis * local + Vector3.UP * (0.08 + height)
...
func _append_kitchen_garden(builder: Dictionary, center: Vector3, size: Vector2, basis: Basis) -> void:
	...
	for row in 3:
		...
		for plant in 5:
			...
			var crop_center := center + basis * Vector3(row_x, 0, z) + Vector3.UP * (0.10 + h * 0.5)
			_append_box(builder, crop_center, Vector3(0.13, h, 0.13), basis, 2 if (row + plant) % 2 == 0 else 3)
...
func _append_herb_garden(builder: Dictionary, center: Vector3, size: Vector2, basis: Basis) -> void:
	...
	for index in 16:
		...
		var herb := center + basis * Vector3(x, 0, z) + Vector3.UP * (0.08 + h * 0.5)
		_append_box(builder, herb, Vector3(0.11, h, 0.11), basis, 2 if index % 3 else 3)
