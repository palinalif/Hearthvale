...
Normalize structural visual geometry to the `0.125` world-space unit across the cottage shell, vegetation, props and terrain presentation. Use the player-approved cottage-only `0.0625` decorative tier for window joinery, shutters, trims, roof edges, flower boxes and flowers. Preserve `0.125` structural dimensions, resize increments and attachment anchors, saved data, and independent native editing resolution. Validate declared cell size, cubic proportions and grid phase after transforms, resizing and duplication. Larger forms use additional cells, not stretched voxels. Include side-by-side visual evidence and meaningful world-space geometry checks before marking this requirement passed.
