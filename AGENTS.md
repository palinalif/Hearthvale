...
- Native terrain and authoritative cottage structure use `VisualGrid.UNIT = 0.125`. Non-terrain presentation—including trees, foliage, flowers, mushrooms, rocks, visible roof tiles/edges, window and door joinery, shutters, trims, and flower boxes—uses cubic `0.0625` cells while preserving its established world dimensions. Structural dimensions, roof authority, resize increments, attachment anchors, collision, placement records, and save data remain on the `0.125` grid. Add/remove cells to refine or resize assets; never stretch cells. Decorative cells are not independently simulated blocks.
