...
- The long-running headless Godot MCP editor can **silently revert an externally edited `.gd` file to its stale in-memory copy** (the edit looks correct on disk, then vanishes before the commit, and the "verified" build ships without it). Re-`grep` edited scripts immediately before committing, and kill the editor process tree before on-device build work.
