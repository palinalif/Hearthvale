...
Trees receive deterministic 15-degree Y rotation from each existing planting record, matching the home's coarse rotation steps. Ground foliage retains deterministic cardinal quarter turns. Both apply equally to the starter scatter and brush placement without expanding the save schema. Foliage and rocks use per-instance color modulation for a restrained stable hue/value range without extra draw batches. Mushroom geometry retains pale stems below the restored warm 7131d22 caps.
