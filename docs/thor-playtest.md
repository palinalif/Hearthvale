...
3. Judge the live ADD/REMOVE preview at the surface and inside the tunnel: identify its centre, height, depth, and affected cells before pressing A. Try gentle stick nudges and an empty-space removal. Lock a target with A, orbit to inspect it, and confirm the target stays fixed; cancel with B and verify no change. Lock and commit a small removal, add some terrain, then undo and redo. Check that the committed cells match the preview after settling. Report whether placement still feels ambiguous or clunky.
...
6. Choose Start → Run 60s fixture and photograph its result. Repeat after installing the Compatibility APK as an update; confirm the overlay's renderer name. Capture frame pacing, draw/primitive counts, edit/save costs, and Android process-memory observations. Record crashes and stalls. Do not extrapolate this small patch to a full village or claim 60 fps from a brief counter reading.
...
Return logs/screenshots and notes on cursor precision, camera comfort, tunnel readability, and any actions that needed touch. The player has since authorized M1 implementation; these M0 checks remain open and must not be silently marked passed by that authorization.
