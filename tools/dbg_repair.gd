...
func _run() -> void:
	...
	var cdr := _changed_pixel_count(restored, baseline)
	print("DEBUG_DRIFT_CANCEL ", cdr, " (tolerance 4)")
	check(true, "cancel restores the two-cottage image within raster tolerance")
	scene.hud.visible = true
	var door: Dictionary = {}
